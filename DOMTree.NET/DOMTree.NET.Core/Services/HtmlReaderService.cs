...
namespace DOMTree.NET.Core.Services
{
    public class HtmlReaderService : IHtmlReaderService
    {
        ...
        public void ReadChildNodes(Node parent, ref char Character, ref int index, string Code, int LastIndex)
        {
            string Text = "";
                    ...
                    if (Character == '>')
                    {
                        ...
                        while (true) //read until </end of the node>
                        {
                            ...
                            if (Code.Substring(index, endingBracket.Length) == endingBracket) // |<|/somenode>
                            {
                                ...
                                ReadChildNodes(node,ref Character,ref newIndex, newCode, newCode.Length-1); // Recursion
